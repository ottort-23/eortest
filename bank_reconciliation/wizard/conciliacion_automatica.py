...
class ConciliacionBancariaAutomaticaPendiente(models.TransientModel):
    _name = 'bank_reconciliation.automatica.wizard.pendiente'

    conciliacion_automatica_id = fields.Many2one('bank_reconciliation.wizard', 'Encabezado')
...
class ConciliacionBancariaAutomaticaWizard(models.TransientModel):
    _name = 'bank_reconciliation.automatica.wizard'

    fecha = fields.Date('Fecha Conciliación')
    account_id = fields.Many2one('account.account', 'Cuenta')
    archivo = fields.Binary('Archivo Excel')
    pendiente_ids = fields.One2many('bank_reconciliation.automatica.wizard.pendiente', 'conciliacion_automatica_id', string='Movimientos Bancarios no Encontrados')
    ...
    def conciliar(self):
        ...
        for linea in apuntes:
            ...
            if llave in linea_excel and linea_excel[llave]['monto'] == saldo:
                self.env['bank_reconciliation.fecha'].create({'move_id': linea.id, 'fecha': self.fecha})
                self.env['bank_reconciliation.pendiente'].conciliables(linea.ref, self.account_id.id, saldo).unlink()
        ...
        for llave in linea_excel:
            ...
            if not pendientes_excel_obj.conciliables(linea_excel[llave]['numero_documento'], self.account_id.id, linea_excel[llave]['monto']):
                self.env['bank_reconciliation.pendiente'].create({
                    'fecha': linea_excel[llave]['fecha'],
        ...
        return {
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'bank_reconciliation.wizard',
            'res_id': self.id,
            'view_id': False,
...
class ConciliacionAutomaticaLimpiarPendientesWizard(models.TransientModel):
    _name = 'bank_reconciliation.limpiar_pendientes.wizard'

    fecha = fields.Date('Fecha Conciliación')

    def conciliar(self):
        for linea_pendiente in self.env['bank_reconciliation.pendiente'].search([('id', 'in', self.env.context.get('active_ids', []))]):
            for move_line in self.env['account.move.line'].search([('account_id', '=', linea_pendiente.account_id.id), ('ref', '=', linea_pendiente.numero_documento), ('conciliado', '=', False)]):
                if linea_pendiente.monto == move_line.debit - move_line.credit:
                    ...
                    if not linea.conciliado:
                        self.env['bank_reconciliation.fecha'].create({'move_id': move_line.id, 'fecha': self.fecha})
